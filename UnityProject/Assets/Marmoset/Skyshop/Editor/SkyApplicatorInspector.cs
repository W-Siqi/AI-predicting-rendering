...
namespace mset
{
	...
	public class SkyApplicatorInspector : Editor
	{
		...
		public override void OnInspectorGUI() {
			Undo.RecordObject(this.target, "Sky Trigger change");
			EditorGUI.BeginChangeCheck();			
			myapp = this.target as mset.SkyApplicator;
			myapp.TriggerIsActive = EditorGUILayout.Toggle(new GUIContent("Use Trigger"), myapp.TriggerIsActive);
			...
			if(EditorGUI.EndChangeCheck()) {
				...
				EditorSceneManager.MarkSceneDirty(myapp.gameObject.scene);
		...
		public void OnSceneGUI(){
			...
			if (mset.SkyApplicatorInspector.triggerEdit && myapp.TriggerIsActive)
			{
				...
				if( dim.center != myapp.TriggerDimensions.center || dim.size != myapp.TriggerDimensions.size ) {
					Undo.RecordObject(this.target, "Sky Trigger Resize");
					myapp.TriggerDimensions = dim;
